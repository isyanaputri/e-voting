...
$votes = $pdo->query("SELECT * FROM votes ORDER BY id ASC")->fetchAll();

// Tambahkan fitur: cari vote by hash
$searchHash = trim($_GET['hash'] ?? '');
if ($searchHash !== '') {
    $stmt = $pdo->prepare("SELECT * FROM votes WHERE vote_hash LIKE ? OR id=?");
    $stmt->execute(["%$searchHash%", (int)$searchHash]);
    $votes = $stmt->fetchAll();
}
?>
<!DOCTYPE html>
<html>
<head>
<title>Verifikasi Vote - TrustVote</title>
...
<p class="subtitle">Verifikasi integritas dan keaslian data voting</p>
</div>
...
<?php if ($searchHash): ?>
<a href="verify.php" class="btn btn-outline-secondary">Reset</a>
<?php endif; ?>
</form>
</div>
...
<p class="subtitle">
<?= $searchHash ? "Tidak ada vote dengan hash/ID tersebut." : "Belum ada data vote." ?>
</p>
</div>
<?php endif; ?>
...
<div class="row g-3">
<div class="col-md-6">
<p><b class="text-warning">Candidate:</b><br>
...
<details>
<summary style="color:#cbd5e1; cursor:pointer;">
🔐 Lihat Data Enkripsi Lengkap
</summary>
...
<a href="result.php" class="btn btn-warning me-2">← Kembali ke Hasil</a>
